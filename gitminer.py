...
output_file_handler = logging.FileHandler(
    "{}git_miner_{}.log".format(LOG_DIR, tstamp))
...
class GithubCli(object):
    ...
    def create_table_list(self, dictionary):
        ...
        language_table.sort(
            key=lambda language_table: language_table[1], reverse=True)
    ...
    def get_repo_details(self, g):
        ...
        log.info(
            f"{colored('Repos', color='blue', attrs=['bold', 'underline'])}:")
        for repo in g.repos:
            g.repo = repo
            ...
            log.info(f" |")
            log.info(
                f" |- {colored(f'Repo Name', color='green')}: {g.repo.name}")
            log.info(
            ...
            if g.repo.language not in g.identified_languages:
                ...
                g.language_dict[g.repo.language] += 1
            log.info(
                f" |- {colored(f'Languages:', color='green')}")
            for language, byte_size in g.languages.items():
                byte_size = bytesto(byte_size, to='k')
                log.info(f"  |")
                log.info(
                    f"   |- {colored(f'Language: {language}', attrs=['bold'])}")
                log.info(
                    f"   |- {colored(f'Byte Size: {byte_size} KB', attrs=['bold'])}")
            log.info(
                f" |- {colored(f'Root Content Files:', color='green')}")
            log.info(f"  |")
            for content in g.contents:
                log.info(
                    f"   | * {colored(f'{content.path}', attrs=['bold'])}")

            _repo = {'org': g.org.login,
    ...
    def print_details(self, g):
        ...
        log.info(tabulate(g.language_table, headers=[
    ...
    def write_csv(self, g):
        ...
        with open(self.csv_file, 'w') as f:
            fieldnames = ['org','org_owner','org_url',
                          'repo_name','repo_lang',
                          'repo_langs','repo_tld']
...
def main(hostname=hostname, all_orgs=all_orgs, org_name=org_name, repo_name=repo_name):
    ...
    if hostname == 'acme':
        ...
        log.error(
            '[.] Please set valid hostname flag(ex: acme) and try again')
        log.error('[*] Exiting...\n')
        sys.exit()

    if not noprompt:
        a = input(
            colored(text=f'[.] Are you sure you want to run this script in {colored(running_url, color="white", attrs=["bold", "underline"])}: (y/n)\n  > ', color="green"))
    ...
    if all_orgs and not hostname:
        ...
        if not noprompt:
            a = input(
                colored(text=f'[.] Are you sure you want to run this script for all orgs: (y/n)\n  > ', color="red"))
            if a != 'y':
...
if __name__ == '__main__':
    try:
        main(hostname=hostname, all_orgs=all_orgs,
